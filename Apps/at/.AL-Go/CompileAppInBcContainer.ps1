...
if ($appFile) {
    ...
    if ($filename -like "Microsoft_System Application_*.*.*.*.app") {
        ...
        Invoke-ScriptInBcContainer -containerName $parameters.ContainerName -scriptblock { Param([string]$packagesFolder)
            ...
            Copy-Item -Path $baseApp -Destination "c:\run\my\testtoolkit\Microsoft_Base Application.app"
            ...
            Copy-Item -Path $applicationApp -Destination "c:\run\my\testtoolkit\Microsoft_Application.app"
    ...
    if ($filename -like "Microsoft_Any.app" -or $filename -like "Microsoft_Any_*.*.*.*.app" -or
        ...
        Copy-Item -Path $appfile -Destination (Join-Path $bcContainerHelperConfig.hostHelperFolder "Extensions\$($parameters.ContainerName)\my\testtoolkit") -Force
    }
}
